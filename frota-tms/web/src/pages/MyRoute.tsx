...
/**
 * Meu roteiro — mesmo padrão visual do Login/FrotaTMS.
 * Layout organizado para abrir bem no celular (app-like), sem mudar a cara do produto.
 */
export function MyRoute() {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-[var(--color-bg)] px-4 pt-[max(1rem,env(safe-area-inset-top))] pb-[max(1.25rem,env(safe-area-inset-bottom))]">
      <button
        type="button"
        onClick={toggleTheme}
        className="absolute top-[max(1rem,env(safe-area-inset-top))] right-4 rounded-md p-2 text-[var(--color-text-muted)] hover:bg-[var(--color-surface)]"
        aria-label="Tema"
      >
        {theme === 'dark' ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
      </button>

      <div className="mx-auto flex w-full max-w-sm flex-1 flex-col pt-10 sm:justify-center sm:pt-0">
        <div className="mb-8">
        ...
        {result && (
          <div
            ref={resultRef}
            className="mt-6 scroll-mt-4 rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-surface)] p-4"
          >
            {!result.found ? (
